...
class HTMotorGUIController:
    ...
    def setup_ui(self):
        ...
        self.connect_btn = ttk.Button(
            connection_frame, text="连接电机", command=self.connect_motors
        )
        ...
        self.disconnect_btn = ttk.Button(
            connection_frame,
            text="断开连接",
            command=self.disconnect_motors,
            state="disabled",
        )
        ...
        self.status_label = ttk.Label(
            connection_frame, text="状态: 未连接", foreground="red"
        )
        ...
        # 电机1角度控制
        ttk.Label(motor1_frame, text="目标角度 (度):").grid(
        ...
        motor1_scale = ttk.Scale(
            motor1_angle_frame,
            from_=-180,
            to=180,
            variable=self.motor1_target_angle,
            orient=tk.HORIZONTAL,
            length=300,
        )
        motor1_scale.grid(row=0, column=0, sticky=(tk.W, tk.E))

        motor1_entry = ttk.Entry(
            motor1_angle_frame, textvariable=self.motor1_target_angle, width=10
        )
        motor1_entry.grid(row=0, column=1, padx=(10, 0))

        # 电机1状态显示
        ttk.Label(motor1_frame, text="当前角度:").grid(
            row=2, column=0, sticky=tk.W, pady=(10, 0)
        )
        self.motor1_pos_label = ttk.Label(
            motor1_frame, text="0.00°", font=("Arial", 12, "bold")
        )
        self.motor1_pos_label.grid(row=3, column=0, sticky=tk.W)

        ttk.Label(motor1_frame, text="当前速度:").grid(
        ...
        ttk.Label(motor1_frame, text="当前力矩:").grid(
            row=6, column=0, sticky=tk.W, pady=(5, 0)
        )
        ...
        ttk.Button(
            motor1_btn_frame, text="停止", command=lambda: self.stop_motor(1)
        ).grid(row=0, column=0, padx=(0, 5))
        ttk.Button(
            motor1_btn_frame, text="读取角度", command=lambda: self.read_motor_angle(1)
        ).grid(row=0, column=1)
        ...
        # 电机2角度控制
        ttk.Label(motor2_frame, text="目标角度 (度):").grid(
            row=0, column=0, sticky=tk.W, pady=(0, 5)
        )
        ...
        motor2_scale = ttk.Scale(
            motor2_angle_frame,
            from_=-180,
            to=180,
            variable=self.motor2_target_angle,
            orient=tk.HORIZONTAL,
            length=300,
        )
        motor2_scale.grid(row=0, column=0, sticky=(tk.W, tk.E))

        motor2_entry = ttk.Entry(
            motor2_angle_frame, textvariable=self.motor2_target_angle, width=10
        )
        motor2_entry.grid(row=0, column=1, padx=(10, 0))

        # 电机2状态显示
        ttk.Label(motor2_frame, text="当前角度:").grid(
            row=2, column=0, sticky=tk.W, pady=(10, 0)
        )
        self.motor2_pos_label = ttk.Label(
            motor2_frame, text="0.00°", font=("Arial", 12, "bold")
        )
        self.motor2_pos_label.grid(row=3, column=0, sticky=tk.W)

        ttk.Label(motor2_frame, text="当前速度:").grid(
            row=4, column=0, sticky=tk.W, pady=(5, 0)
        )
        self.motor2_vel_label = ttk.Label(motor2_frame, text="0.00 rad/s")
        self.motor2_vel_label.grid(row=5, column=0, sticky=tk.W)

        ttk.Label(motor2_frame, text="当前力矩:").grid(
            row=6, column=0, sticky=tk.W, pady=(5, 0)
        )
        ...
        ttk.Button(
            motor2_btn_frame, text="停止", command=lambda: self.stop_motor(2)
        ).grid(row=0, column=0, padx=(0, 5))
        ttk.Button(
            motor2_btn_frame, text="读取角度", command=lambda: self.read_motor_angle(2)
        ).grid(row=0, column=1)
        ...
        ttk.Button(
            global_frame, text="停止所有电机", command=self.stop_all_motors
        ).grid(row=0, column=1, padx=(0, 10))
        ttk.Button(global_frame, text="回到零位", command=self.move_to_zero).grid(
            row=0, column=2, padx=(0, 10)
        )
        ttk.Button(
            global_frame, text="读取所有角度", command=self.read_all_angles
        ).grid(row=0, column=3)
        ...
        ttk.Button(file_btn_frame, text="保存轨迹", command=self.save_trajectory).grid(
            row=0, column=0, padx=(0, 5)
        )
        ttk.Button(file_btn_frame, text="加载轨迹", command=self.load_trajectory).grid(
            row=0, column=1, padx=(0, 5)
        )
        ttk.Button(file_btn_frame, text="清除轨迹", command=self.clear_trajectory).grid(
            row=0, column=2, padx=(0, 5)
        )

        # 录制状态显示
        self.record_status_label = ttk.Label(
            record_frame, text="状态: 未录制", foreground="blue"
        )
    ...
    def connect_motors(self):
        ...
        try:
            ...
            usb_hw = USB_HW(1000000, 5000000, "693D3DE86DF5940C8BC74A5B46A3CE2E")
    ...
    def start_continuous_control_mode(self):
        ...
        try:
            ...
            # 启动连续控制线程
            self.continuous_control_thread = threading.Thread(
                target=self.continuous_control_loop, daemon=True
            )
    ...
    def continuous_control_loop(self):
        ...
        try:
            ...
            print(
                f"连续控制开始: 初始目标角度 电机1={self.last_target1}°, 电机2={self.last_target2}°"
            )
            ...
            print(f"连续控制循环错误: {e}")
            self.root.after(
                0, lambda: messagebox.showerror("错误", f"连续控制过程中出错: {str(e)}")
            )
        finally:
            self.is_continuous_control = False
    ...
    def update_ui_state(self):
        ...
        if self.is_connected:
            ...
            self.enable_btn.config(
                state="normal" if not self.is_enabled else "disabled"
            )
            ...
            if (
                ...
                and not self.is_continuous_control
            ):
    ...
    def start_monitoring(self):
        ...
        self.monitoring_thread = threading.Thread(
            target=self.monitor_motors, daemon=True
        )
        ...
        while not self.stop_monitoring and self.is_connected:
            try:
                if self.motor1 and self.motor2:
                    ...
                    if (
                        self.is_recording
                        and current_time - last_record_time >= self.recording_interval
                    ):
                        ...
                        self.recorded_trajectory.append(
                            [relative_time, motor1_pos, motor2_pos]
                        )
                        ...
                        self.root.after(
                            0,
                            lambda: self.trajectory_info_label.config(
                                text=f"轨迹点数: {len(self.recorded_trajectory)}"
                            ),
                        )
    ...
    def stop_recording(self):
        ...
        try:
            ...
            self.record_status_label.config(
                text=f"状态: 录制完成 ({duration:.1f}秒)", foreground="green"
            )
            self.trajectory_info_label.config(
                text=f"轨迹点数: {len(self.recorded_trajectory)}"
            )
    ...
    def start_playback(self):
        ...
        try:
            ...
            self.record_status_label.config(
                text="状态: 正在回放...", foreground="orange"
            )

            # 启动回放线程
            self.playback_thread = threading.Thread(
                target=self.playback_trajectory, daemon=True
            )
    ...
    def playback_trajectory(self):
        ...
        try:
            ...
            for i, point in enumerate(self.recorded_trajectory):
                ...
                self.root.after(
                    0,
                    lambda p=progress: self.record_status_label.config(
                        text=f"状态: 回放中... ({p:.1f}%)", foreground="orange"
                    ),
                )
            ...
            if not self.stop_playback:
                self.root.after(
                    0,
                    lambda: self.record_status_label.config(
                        text="状态: 回放完成", foreground="green"
                    ),
                )
            ...
            self.root.after(
                0, lambda: messagebox.showerror("错误", f"回放过程中出错: {str(e)}")
            )
            self.is_playing = False
            ...
            if filename:
                trajectory_data = {
                    ...
                    "metadata": {
                        ...
                        "duration": self.recorded_trajectory[-1][0]
        ...
        try:
            ...
            if filename:
                ...
                if len(self.recorded_trajectory) > 0:
                    ...
                    self.record_status_label.config(
                        text=f"状态: 已加载轨迹 ({duration:.1f}秒)", foreground="blue"
                    )
