...
class MuJoCoICARMSimulation:
    def __init__(self, urdf_path=None, update_rate=30.0):
        ...
        if urdf_path is None:
            ...
            urdf_path = "/Users/lr-2002/project/instantcreation/ic_can/urdf/urdf/ic_arm_cnc.urdf"
                ...
                if isinstance(position_data, dict) and "rad" in position_data:
                    ...
                    if angle_rad is not None:
                        try:
                            ...
                            print(
                                f"  {motor_name} -> joint_idx={joint_idx}: {old_pos:.4f} -> {angle_rad:.4f}"
                            )
                        except Exception as e:
    ...
    def print_status(self, ic_arm_positions):
        ...
        print("=" * 60)
        print(
            f"{'Motor':<8} {'IC ARM (rad)':<15} {'IC ARM (deg)':<15} {'MuJoCo (rad)':<15}"
        )
        print("-" * 60)
    ...
    def run_with_viewer(self):
        ...
        try:
            ...
            with viewer.launch_passive(self.model, self.data) as v:
                ...
                v.opt.frame = (
                    mj.mjtFrame.mjFRAME_WORLD
                )  # Show world frame and body frames
                ...
                v.cam.elevation = (
                    -elevation
                )  # Negative because MuJoCo uses different convention
...
def main():
    ...
    parser.add_argument(
        "--headless", action="store_true", help="Run without viewer (headless mode)"
    )
    parser.add_argument(
        "--duration",
        type=float,
    ...
    parser.add_argument(
        "--rate", type=float, default=30.0, help="Update rate in Hz (default: 30)"
    )
