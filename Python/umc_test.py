...
def initialize_unified_motor_system():
	...
	try:
		# 创建USB硬件接口
		print("正在初始化USB硬件接口...")
		usb_hw = usb_class(1000000, 5000000, "your_device_sn")
		...
		# 添加达妙电机协议
		print("初始化达妙电机协议...")
		motor_control = Motor_Control(1000000, 5000000, "F561E08C892274DB09496BCC1102DBC5", damiao_data)
		...
		for i in range(6):
			motor_info = MotorInfo(
				...
				kp=damiao_data[i].kp,
				kd=damiao_data[i].kd,
				limits=dm_limit[damiao_data[i].motorType.value]  # [pos_limit, vel_limit, torque_limit]
			)
			success = manager.add_motor(i+1, 'damiao', motor_info, can_id=damiao_data[i].can_id)
			if success:
	...
	try:
		...
		for i in range(1, 9):
			...
			if motor:
				...
				print(f"电机 {i}: pos={state['position']:.3f}, vel={state['velocity']:.3f}, tau={state['torque']:.3f}")
		...
		while (time.time() - start_time) < duration:
			...
			if loop_count % 1000 == 0:
				...
				for i in range(1, 3):  # 只打印前两个电机的状态
					motor = manager.get_motor(i)
					if motor:
						print(f"  电机{i}: pos={motor.get_position():.3f}, vel={motor.get_velocity():.3f}")
...
def read_only_mode(manager):
	...
	try:
		while True:
			...
			# 格式化显示
			print(f"{'ID':<3} {'Type':<7} {'Position(rad)':<13} {'Velocity(rad/s)':<15} {'Torque(Nm)':<12} {'Error':<5}")
			print("-" * 60)
			
			for data in motor_data:
				status_icon = "✓" if data['error'] == 0 else "✗"
				print(f"{data['id']:<3} {data['type']:<7} {data['position']:<13.4f} {data['velocity']:<15.4f} {data['torque']:<12.4f} {status_icon:<5}")
